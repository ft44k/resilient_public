...
PACKAGE_NAME = "tanium"
FUNCTION_NAME = "tanium_online_check"
...
def call_tanium_online_check_function(circuits, function_params, timeout=10):
    # Fire a message to the function
    evt = SubmitTestFunction("tanium_online_check", function_params)
    circuits.manager.fire(evt)
    event = circuits.watcher.wait("tanium_online_check_result", parent=evt, timeout=timeout)
    assert event
...
class TestTaniumOnlineCheck:
    """ Tests for the tanium_online_check function"""
    ...
    def test_success(self, circuits_app, tanium_endpoint, tanium_user, tanium_password, tanium_host, tanium_port, tanium_pytan_loc, expected_results):
        ...
        results = call_tanium_online_check_function(circuits_app, function_params)
