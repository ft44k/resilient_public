...
PACKAGE_NAME = "tanium"
FUNCTION_NAME = "tanium_get_installed_apps"
...
def call_tanium_get_installed_apps_function(circuits, function_params, timeout=10):
    # Fire a message to the function
    evt = SubmitTestFunction("tanium_get_installed_apps", function_params)
    circuits.manager.fire(evt)
    event = circuits.watcher.wait("tanium_get_installed_apps_result", parent=evt, timeout=timeout)
    assert event
...
class TestTaniumGetInstalledApps:
    """ Tests for the tanium_get_installed_apps function"""
