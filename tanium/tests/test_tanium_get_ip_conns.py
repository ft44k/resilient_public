...
PACKAGE_NAME = "tanium"
FUNCTION_NAME = "tanium_get_ip_conns"
...
def call_tanium_get_ip_conns_function(circuits, function_params, timeout=10):
    # Fire a message to the function
    evt = SubmitTestFunction("tanium_get_ip_conns", function_params)
    circuits.manager.fire(evt)
    event = circuits.watcher.wait("tanium_get_ip_conns_result", parent=evt, timeout=timeout)
    assert event
...
class TestTaniumGetIpConns:
    """ Tests for the tanium_get_ip_conns function"""
    ...
    def test_success(self, circuits_app, tanium_endpoint, incident_id, tanium_user, tanium_password, tanium_host, tanium_port, tanium_pytan_loc, expected_results):
        ...
        results = call_tanium_get_ip_conns_function(circuits_app, function_params)
